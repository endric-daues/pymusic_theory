# This Code is taken from: https://github.com/dmpayton/python-fretboard/tree/master
# I just took what I needed, and was having trouble with the landscape orientation.
...
class Fretboard(object):
    def __init__(
        self, strings=6, frets=(0, 5), inlays: Tuple[int] = (3, 5, 7, 9), style=None
    ):
        self.strings = [
            attrdict.AttrDict(
                {
    ...
    def draw_inlays(self):
        for index, fret in enumerate(self.frets):
            ...
            if self.style.drawing.orientation == "portrait":
                x = self.style.drawing.spacing - (self.style.inlays.radius * 8)
                y = self.layout.y + inlay_dist
            else:
                x = self.layout.x + inlay_dist
                y = self.layout.y + self.layout.height + (self.style.inlays.radius * 8)
